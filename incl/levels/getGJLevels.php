...
if($type != 10 AND !isset($_POST["gauntlet"])){
   ...
   $str = $ep->remove($_POST["str"]);
	...
	if($type==1){
		...
		$countquery = "SELECT count(*) FROM levels ". $additional . "";
	}
	if($type==2){
	...
	if($type==3){ //TRENDING
		...
		$countquery = "SELECT count(*) FROM levels WHERE uploadDate > ".$uploadDate . " " . $additionalnowhere . "";
	}
	if($type==4){ //RECENT
		$countquery = "SELECT count(*) FROM levels ". $additional . "";
    ...
    if($type==5){
		$countquery = "SELECT count(*) FROM levels WHERE userID = '".$str."' " . $additionalnowhere . "";
	...
	if($type==6 OR $type==17){ //featured
		$countquery = "SELECT count(*) FROM levels WHERE NOT starFeatured = 0 ".$additionalnowhere."";
		$query = "SELECT * FROM levels WHERE NOT starFeatured = 0 ".$additionalnowhere." ORDER BY gameVersion DESC, uploadDate DESC LIMIT 10 OFFSET $lvlpagea";
	}
	if($type==16){ //HALL OF FAME
		$countquery = "SELECT count(*) FROM levels WHERE NOT starEpic = 0 ".$additionalnowhere."";
		$query = "SELECT * FROM levels WHERE NOT starEpic = 0 ".$additionalnowhere." ORDER BY uploadDate DESC LIMIT 10 OFFSET $lvlpagea";
	}
	if($type==7){ //MAGIC
		$countquery = "SELECT count(*) FROM levels WHERE objects > 9999 ". $additionalnowhere . "";
		$query = "SELECT * FROM levels WHERE objects > 9999 ". $additionalnowhere . " ORDER BY uploadDate DESC LIMIT 10 OFFSET $lvlpagea";
	}
	if($type==11){
		$countquery = "SELECT count(*) FROM levels WHERE NOT starStars = 0 ".$additionalnowhere."";
	...
	if($type==12){ //FOLLOWED
		...
		$countquery = "SELECT count(*) FROM levels WHERE extID = ".$whereor." ".$additionalnowhere."";
	...
	for ($x = 0; $x < $levelcount; $x++) {
		...
		if($level1["levelID"]!=""){
			if($type == 0 AND $str == $level1["levelID"] AND $level1["levelID"] == $level1["levelName"]){
				$totallvlcount--;
			}
			if($x != 0){
				echo "|";
