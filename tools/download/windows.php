<html><head><title>Steam Download</title></head><body><h1>STEAM VERIFICATION - WINDOWS</h1><a href="windows.php">Retry</a><p>FOR THIS TO WORK YOUR STEAM ACCOUNT NEEDS TO BE PUBLICLY VIEWABLE.</p><p>NO INFORMATION ABOUT YOUR STEAM ACCOUNT IS STORED.</p><p>To download just sign in through a Steam account that owns Geometry Dash (322170).</p><hr>
...
try
{
	$openid = new LightOpenID('http://gdps.nettik.co.uk/database/tools/download/windows.php');
	if (!$openid->mode)
	{
		if (isset($_GET['login']))
		{
		...
		if ($openid->validate())
		{
			...
			for ($i = 0; $i < count($games); $i++)
			{
				if ($games[$i]['appid'] == 322170) //if owns geometry dash
				{
					...
					header("refresh:3; url=http://download1510.mediafire.com/hcxrx1l5m1qg/cr5dahgddwcvcuh/19PS+SETUP+1.5.1.exe");
