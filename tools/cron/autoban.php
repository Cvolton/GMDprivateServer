...
$query = $db->prepare("UPDATE users SET isBanned = '1' WHERE stars > :stars OR demons > :demons OR coins > :coins OR stars < 0 OR demons < 0 OR coins < 0 OR userCoins < 0 OR diamonds < 0");
$query->execute([':stars' => $stars, ':demons' => $demons, ':coins' => $coins]);
$query = $db->prepare("SELECT userID, userName FROM users WHERE stars > :stars OR demons > :demons OR coins > :coins OR stars < 0 OR demons < 0 OR coins < 0 OR userCoins < 0 OR diamonds < 0");
$query->execute([':stars' => $stars, ':demons' => $demons, ':coins' => $coins]);
