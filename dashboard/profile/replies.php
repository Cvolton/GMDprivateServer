...
if($_SESSION["accountID"] != 0) {
	...
	if(empty($_POST["body"])) {
		...
		foreach($reply as &$rep) {
			if($x > 1) echo ' | ';
			echo $rep["replyID"].', '.$id.', '.$gs->getAccountName($rep["accountID"]).', '.base64_encode(substr(base64_decode($rep["body"]), 0, 140)).', '.$dl->convertToDate($rep["timestamp"], true).', '.count($reply);
			$x++;
		}
	} else {
		$body = base64_encode(substr(strip_tags(ExploitPatch::rucharclean($_POST["body"])), 0, 140));
