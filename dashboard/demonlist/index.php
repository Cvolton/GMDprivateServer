...
if(!empty($_POST["sr"]) AND is_numeric($_POST["sr"])) {
	...
	if(!empty($_POST["atts"] AND !empty($_POST["ytlink"]))) {
		...
		$dl->printSong('<div class="form">
		<a href="demonlist"><h1>'.sprintf($dl->getLocalizedString('submitRecordForLevel'), $gs->getLevelName($_POST["sr"])).'</h1></a>
		<form class="form__inner" method="post" action="">
			...
			<button style="margin-top:5px;margin-bottom:5px" type="submit" name="sr" value="'.$_POST["sr"].'" class="btn-song">'.$dl->getLocalizedString('submit').'</button>
		</form></div>', 'browse');
	}
} else {
	if(!empty($_POST["change"]) AND $gs->checkPermission($_SESSION["accountID"], "demonlistAdd")) {
		...
		<a href="demonlist"><h1>'.$dl->getLocalizedString('addDemon').'</h1></a>
		<form class="form__inner" method="post" action="">
			...
			<button style="margin-top:5px;margin-bottom:5px" type="submit" name="change" value="1" class="btn-song">'.$dl->getLocalizedString('add').'</button>
		</form></div>', 'browse');
		...
		$table = '<table style="position:relative" class="table table-inverse"><tr><th>#</th><th>'.$dl->getLocalizedString("levelid").'</th><th>'.$dl->getLocalizedString("levelname").'</th><th>'.$dl->getLocalizedString("atts").'</th><th>'.$dl->getLocalizedString("status").'</th></tr>';
		...
		$table .= '<a class="btn-block" style="position: absolute;padding: 15px;top: 6vh;cursor: pointer;left: 13vw;width: max-content;font-size: 21px" href="demonlist"><i class="fa-solid fa-arrow-left"></i></a></table>';
		...
		foreach($list as &$demons) {
			...
			if($_SESSION["accountID"] != 0) {
				...
				if(!empty($sub)) {
					...
					elseif($sub["approve"] == -1) $submitbtn .= '<button style="width: max-content;padding:5 13" class="btn-primary" disabled>'.$dl->getLocalizedString('recordDenied').' <i class="fa-solid fa-xmark"></i></button>';
					else $submitbtn .= '<button style="width: max-content" class="btn-primary" disabled>'.$dl->getLocalizedString('recordSubmitted').' <i class="fa-solid fa-check"></i></button>';
				}
				else $submitbtn .= '<form method="post" action="" style="margin:0"><button style="width: max-content;margin-right:10px" name="sr" class="btn-primary" value="'.$demons["levelID"].'">'.$dl->getLocalizedString('submitRecord').'</button></form>';
			} else $submitbtn .= '';
