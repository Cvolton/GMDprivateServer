...
if(!empty($clan)) {
    if(isset($_POST["settings"]) AND $_POST["settings"] == 1 AND $clan["clanOwner"] == $_SESSION["accountID"]) {
        if(!isset($_POST["ichangedsmth"])) {
			...
			if($clan["isClosed"] == 1) {
				...
				$pending = '<div><h2 style="text-align:right;margin:0;margin-bottom: 3px;">'.$dl->getLocalizedString("pendingRequests").'</h2>
                                        ...
                                        <button style="height: max-content;" class="btn-rendel" type="button" onclick="a(\'clan/'.$clan['clan'].'/settings\', true, true, \'POST\', false, \'pending\')">'.$dl->getLocalizedString("pendingRequests").'</button>
                                    </form></div>';
			}
		...
		<script>
            ...
            $name = base64_encode(ExploitPatch::remove(str_replace(' ', '', $_POST["clanname"]), 20));
            $desc = base64_encode(ExploitPatch::remove($_POST["clandesc"], 255));
            $color = ExploitPatch::remove(mb_substr($_POST["clancolor"], 1), 6);
            ...
            if(!empty($name) AND !empty($color) AND is_numeric($isClosed)) {
                ...
                $clan["clan"] = base64_decode($name);
                $clan["desc"] = base64_decode($desc);
                $clan["color"] = $color;
